...
<head>
	...
	<title>국토정보포털 통합관리자 - Q&amp;A수정</title>
			...
			<form action="updateQnA.do" method="post">
				<fieldset>
					...
					<div class="table-vertical">
						<table>
							<caption class="blind">Q&amp;A 수정</caption>
							<colgroup>
							...
							<tbody>
								<tr>
									...
									<td>
										<select id="w-type" name="division_id">
											<c:forEach items="${division}" var="division">
											<option value="${division.id}" <c:if test="${qna.division == division.name}">selected</c:if>>${division.name}</option>
											</c:forEach>
									</select>
									</td>
								</tr>
								<tr>
									...
									<td><input type="text" id="w-title" name="title" value="${qna.title}" style="width:550px;"></td>
								</tr>
								<tr>
									<th scope="row"><label for="w-content">내용</label></th>
									<td><textarea type="text" id="w-content" name="content" value="" rows="20" style="width:550px;">${qna.content}</textarea></td>
								</tr>
								<tr>
					...
					<input type="hidden" id="w-id" name="id" value="${qna.id}">
				</fieldset>
				<div class="bt-area text-right">
				<button type="button"  onclick="location.href='qnaPageList.do'" class="bt reset">취소</button>
