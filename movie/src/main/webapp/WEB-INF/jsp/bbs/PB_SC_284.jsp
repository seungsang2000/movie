...
<head>
	...
	<title>국토정보포털 통합관리자 - 공지사항수정</title>
		...
		<section id="content" class="content">
			...
			<form action="updateNotice.do" method="post">
				<fieldset>
						...
						<table>
							...
							<tbody>
								<tr>
									...
									<td>
										<select id="w-type" name="division">
											<option value="국토정보플랫폼" <c:if test="${notice.division=='국토정보플랫폼'}">selected</c:if>>국토정보플랫폼</option>
											<option value="GIS 서비스" <c:if test="${notice.division=='GIS 서비스'}">selected</c:if>>GIS 서비스</option>
											<option value="데이터 분석"<c:if test="${notice.division=='데이터 분석'}">selected</c:if>>데이터 분석</option>
										</select>
									</td>
								</tr>
								<tr>
									<th scope="row"><label for="w-title">제목</label></th>
									<td><input type="text" id="w-title" name="title" value="${notice.title}" style="width:550px;"></td>
								</tr>
								<tr>
									<th scope="row"><label for="w-content">내용</label></th>
									<td><textarea type="text" id="w-content" name="content" value="" rows="20" style="width:550px;">${notice.content}</textarea></td>
								</tr>
								<tr>
					...
					<input type="hidden" id="w-id" name="id" value="${notice.id}">
				</fieldset>
